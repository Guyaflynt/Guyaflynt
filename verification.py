...
ftypes=['torn', 'hail', 'wind', 'sigwind', 'sighail', 'sigtorn']	#SPC convective outlook forecast types (CONTROLLED BY RUNVERIF.PY!!)
datadir= '/home/cgodfrey/research/spcverif/grib/'					#Location of SPC convective outlook files
...
distdir="/home/cgodfrey/utilities/"									#The location of the Sodano distance calculator (Python)
#******************
#******************

#TEST BLOCK FOR SHORTENED USER MODS
#print "***  TESTING!!!  ***"
#fcst_times=['1200','1300','1630','2000']
#*************************
...
if testrun:
	print "\n*************** TEST RUN ****************"
	print "******** READING ONLY FIVE FILES ********\n"
	import time

#Create output file and write header (file name shows verification grid size)
contout=open("contingency_adjpts"+str(adjpts)+"_"+ftypes[0]+".dat","w")	#Open output file for contingency table and other data for each forecast period
print >> contout, "YYYYMMDDHHMM ; type ; a, b, c, d; fprob : nfcst, nobs; [repeat last item for all forecast probabilities]"

#Create output file for annual totals of forecasts/observations
annver=open("annual_adjpts"+str(adjpts)+"_"+ftypes[0]+".verif","w")
print >> annver, "YEAR; type; time; fprob : nfcst, nobs; [repeat last item for all forecast probabilities]"

gotlatlon=False
verif={}
verif_date={}

#Import SPC observations. Files include date, time, which state the event occured in, and lat/lon coordinates. Tornado files include an EF rating. Hail files include hail size. Wind files include the highest estimated wind speed. 
...
for file in files:
	with open(file) as excelfile:
		#print "Reading ",file
		#excelfile= open('2011_torn.csv','r')
		ftype=file[-8:-4]
		...
		for line in lines:
			colms= line.split(',')
			#print colms
			daymonyr=colms[0].split('/')
			hms=colms[1].split(':')
			mon=string.atoi(daymonyr[0])
			day=string.atoi(daymonyr[1])
			yr=string.atoi(daymonyr[2])
			hour=string.atoi(hms[0])
			minute=string.atoi(hms[1])
			sec=string.atoi(hms[2])
			d=datetime.datetime(yr,mon,day,hour,minute,sec)		#The naive date/time
			#if testrun and verbose: print "Raw date: ",d,ftype
			...
			if zone.strip()=='?':
				...
				zone=string.atoi(zone)
			if zone==9:
				#Time zone is GMT. Make no adjustment.
				local=pytz.timezone("UTC")
			elif zone==3:
				#-----------------------------
				#As awesome as this code is, it accounts for daylight saving time, but the time zone
				#	is Central Standard Time (i.e., there is no CDT).
			...
			fulldate=utc_dt.strftime('%Y%m%d%H%M')			#Convert date to YYYYMMDDHHMM format

			#Code you might need
			#newtime=dt.strptime(datetime,'%Y%m%d%H%M')	#Convert the date to a datetime object
			#newt=newtime+timedelta(hours=-1.0*tzoffset)	#Subtract tzoffset hours
			#newdt=newt.strftime('%Y%m%d%H')				#Convert date to YYYYMMDDHH format
			...
			magn.append(string.atof(colms[4]))
			strtlat.append(string.atof(colms[5]))
			strtlon.append(string.atof(colms[6]))
...
for ftime in fcst_times:
	...
	for ftype in ftypes:
		...
		for year in years:
			...
			#print year
			files=glob.glob(datadir+year+"otlk/"+ftype+"_day1_grib2_"+ftime+"_*")

			if testrun:
				files=files[0:ntestfiles]

			#**************************************************************************************************
			#Remove the initial outlook from consideration if it has been ammended. This probably gives the SPC 
			#	better verification scores, but this approach uses only the archived outlook available online.
			#	See the 0100 UTC 28 February 2011 outlook as an example.
			#**************************************************************************************************
			fname=[]
			i=-1
			for spcfile in sorted(files):
				i+=1
				print i,spcfile
			...
			for file in sorted(files):
				...
				#Test grid distances and distance timing
				#print "Shape of grid is: ",np.shape(lat)
				#command=distdir+"./sodano.exe "+str(lon[230,250])+" "+str(lon[230,251])+" "+str(lat[230,250])+" "+str(lat[230,251])
				#start=time.time()
				#os.system(command+" > dist.tmp")
				#with open("dist.tmp") as distfile:
				#	line=distfile.readline()
				#	output=line.split()
				#	dist=string.atof(output[0])
				#print "Timing: ",time.time()-start
				#print "Fortran: ",dist
				#start=time.time()
				#Use the Python version of Sodano's inverse
				#Usage: dist,alpha12,alpha21=distance(ystart,yend,xstart,xend,3)
				#dist,alpha12,alpha21=distance(lat[230,250],lat[230,251],lon[230,250],lon[230,251],3)
				#print "Timing: ",time.time()-start
				#print "Python: ",dist
				#sys.exit()
				...
				print "Verifying forecasts for ",date
				...
				if ftime != '0100':
					...
					try:
						nrpts=len(reports[ftype][nextdate][0])
						noreport=False
						noreport_d2=False	#Calendar day 2 reports
					except:
						#...and there are no reports on the next calendar day.
						noreport=True
						noreport_d2=True		#Calendar day 2 reports
				else:
				...
				if noreport:
					...
					if date==nextdate:
						...
						if not noreport_d1 and not noreport_d2:
					...
					for rptdate in set([date,nextdate]):	#Loops through unique date values only
						...
						for r in range(len(reports[ftype][rptdate][0])):
							...
							countit=False
							if ftime == "0100":
								if (rptdate==date) and ((string.atoi(rpttime) >= string.atoi(ftime)) and (string.atoi(rpttime) < 1200)):
									countit=True
							else:
								if ((rptdate==date) and (string.atoi(rpttime) >= string.atoi(ftime))) or ((rptdate==nextdate) and (string.atoi(rpttime) < 1200)):
									countit=True
									...
									for igd in range(i-adjpts,i+adjpts+1):
										...
										for jgd in range(j-adjpts,j+adjpts+1):
											...
											#The nearest 6x4 and 4x6 is also always inside withindist (for 25 miles).
												...
												#Here is the old Fortran version. It's TOO SLOW!
												#command=distdir+"./sodano.exe "+str(lons[igd,jgd])+" "+str(lons[i,j])+" "+str(lats[igd,jgd])+" "+str(lats[i,j])
												##start=time.time()
												#os.system(command+" > dist.tmp")
												#with open("dist.tmp") as distfile:
												#	line=distfile.readline()
												...
												#
												#-------------------
												#This works great, but the subprocess command opens a shell with an equal amount of memory.
												#	This doesn't get very far through the SPC data before there is insufficient memory on Blizzard!
												#NEED THIS PREVIOUSLY: import subprocess as sub
												#command=distdir+"./sodano.exe "+str(lons[igd,jgd])+" "+str(lons[i,j])+" "+str(lats[igd,jgd])+" "+str(lats[i,j])
												#m = sub.Popen(command,stdout=sub.PIPE,stderr=sub.PIPE,shell=True)
				...
				#TEST
				#print "This means < 2%: --"
				#print "Lats and lons of probs=='--'"
				#print np.shape(lats)
				#print np.shape(probs)
				#print probs.dtype
				#np.set_printoptions(threshold='nan')
				#print ftime,ftype,year,date
				#print probs
				#print probs[0,0]
				#print probs.mask
				#print "^^^MASK"
				...
				#	print "Yup, it's text."
				#else:
				#	print "Nope, it's not text."
				#print probs[0,0]+0.4
				#print "NEW ATTEMPTS >>>"
				#start=time.time()
				#probset=np.unique(probs.compressed())
				#print "Compressed timing: ",time.time()-start
				#print "UNIQUE SET: ",probset
				#print probs.compressed()
				#print "^^That was probs.compressed()."

				#Make a set of all unique forecast probabilities in this forecast period
				try:
				...
				print >> contout, date+ftime,";",ftype,";",a,",",b,",",c,",",d,";",
				...
				for fprob in sorted(verif_date[date][ftype][ftime]):
					print >> contout, fprob,":",verif_date[date][ftype][ftime][fprob][0],",",verif_date[date][ftype][ftime][fprob][1],";",
				print >> contout
			...
			#Print annual totals
			print >> annver, year,";",ftype,";",ftime,";",
			for fprob in sorted(verif[ftype][ftime]):
				print >> annver, fprob,":",verif[ftype][ftime][fprob][0],",",verif[ftype][ftime][fprob][1],";",
			print >> annver
